...
if uploaded_file is not None:
    ...
    "Data Analyst": 
            ...
            "nlp", "big data", "ai", "statistics", "statistical analysis",
            "sql", "mongodb", "mysql", "postgresql", "nosql", "sqlite",
            "python", "r", "excel", "power bi", "tableau",
            "etl", "data warehousing", "apache spark", "hadoop"
            ]}}]}],

    "HR Manager": 
            [{"label": "SKILL", "pattern": [{"LOWER": {"IN": [
            "recruitment", "hiring", "onboarding", "employee relations",
            "performance management", "training and development", "hr policies",
            "labor laws", "compliance", "payroll", "benefits administration",
            "hr analytics", "organizational development", "diversity and inclusion",
            "conflict resolution", "talent management", "succession planning",
            "hr software", "workday", "sap successfactors", "bamboohr"
            ]}}]}],

    "Web Developer": 
            [{"label": "SKILL", "pattern": [{"LOWER": {"IN": [
            "html", "css", "javascript", "node.js", "react", "angular", "vue.js",
            "django", "flask", "ruby on rails", "spring boot", "asp.net", "express.js",
            "sql", "mongodb", "mysql", "postgresql", "nosql", "sqlite",
            "docker", "kubernetes", "jenkins", "ci/cd",
            "serverless", "microservices", "google cloud", "aws", "azure",
            "ui design", "ux design", "wireframing", "responsive design"
            ]}}]}],

    "App Developer": 
            [{"label": "SKILL", "pattern": [{"LOWER": {"IN": [
            "android", "ios", "flutter", "react native", "xamarin",
            "swift", "kotlin", "java", "c#", "unity3d", "unreal engine",
            "api integration", "mobile ui/ux", "performance optimization",
            "testing frameworks", "firebase", "google play console", "app store"
            ]}}]}]
    }

    ruler.add_patterns(job_and_skill[selected_role])

    original_set = extract_skills(text)
    print('Original text:',original_set)
    unique_skills = []

    for item in original_set:
        lower_item = item.lower()
        if lower_item not in (x.lower() for x in unique_skills):
            unique_skills.append(item)
    print(unique_skills)
    print()
    unique_skills = ','.join(unique_skills)
    print(unique_skills)
